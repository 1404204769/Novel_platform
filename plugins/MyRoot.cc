...
void MyRoot::initAndStart(const Json::Value &config)
{
	...
	try{
		...
		this->ConfigDirectory = this->WorkDirectory + "/../plugins/config.json";
		MyJsonPtr->readFileJson(this->config, this->ConfigDirectory);
	}catch(Json::Value e)
	{
...
void MyRoot::close()
{
	try{
		...
		MyJsonPtr->SaveConfig(this->config, this->ConfigDirectory);
		cout << "准备开始停止应用" << endl;
		app().quit();
